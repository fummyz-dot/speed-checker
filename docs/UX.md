...
The homepage combines an athletics scoreboard with editorial typography: deep green-black, off-white text, muted rules, and a restrained salmon accent for primary actions and the user's race lane. Sections and record columns use rules and whitespace instead of nested rounded cards. The brand and selected headings use system serif fonts; body text and controls remain system sans-serif, with tabular monospace measurement values. No external fonts are loaded. The compact hero leads directly into measurement, race, and quality details without a viewport-height gap. Standalone articles and trust pages share this palette, serif brand/headings, and ruled sections, with sans-serif prose limited to 50rem. Related guides use underlined index links rather than rounded cards. The ranking page presents its existing totals, ranks, and speeds as tabular record rows; Lab reports allow tables and unchanged source charts to extend beyond the prose column. Decorative English eyebrows are omitted except for the Lab publication label. Run retains its existing design. Content, navigation, SEO metadata, and measurement/ranking behavior are unchanged.
...
- the provisional Download live meter continues through upload using the last measured Download value, without implying that Download is measured again;
...
The `/run/` time selector is a localhost-only gameplay tool. On production-like hosts, the initial `BOOT` and verified `READY` states contain no horse, jockey, obstacles, or test-time choices. `READY` displays the server-verified `runTimeSec` to one decimal place; the browser never derives it from the score. `START RUN` is the explicit gesture that starts the optional Web Audio BGM and passes that same time to the game. The ticket is consumed after verification, so reload, direct re-entry, or BFCache restoration returns to the measurement page and requires a new successful ranking participation. The production result actions are「リトライ」and「結果に戻る」: retry reuses the verified time held in memory, while return uses a separate one-shot source measurement ID to restore the matching browser-local result at `/` without reopening ranking submission.
