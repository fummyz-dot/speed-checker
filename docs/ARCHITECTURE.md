...
Run tickets use HMAC-SHA256 with the dedicated `RUN_TICKET_HMAC_SECRET`, a 30-minute TTL, a cryptographically random 32-byte nonce, and a fixed `net-speed-run` purpose. They contain the mapped Run time but not the raw score, measurement, client IP, connection metadata, or condition label. Verification through `POST /api/run-ticket/verify` is stateless and allows replay until expiry; it does not call the ranking service or use persistence.

After an explicit click on the completed-measurement Run card, the browser sends only the five allow-listed measurement fields, stores the issued ticket and expiry in `sessionStorage`, and navigates to the independently built `/run/` static assets. On non-local hosts, `/run/` starts in `BOOT`, verifies the stored ticket, and passes only the verified time to `window.NetSpeedRun.start(runTimeSec)`. Missing, invalid, or expired tickets return to `/`; service failure remains on an error screen with a manual return link. Raw score and time query parameters are ignored. Localhost keeps the development-only time selector, while production never enters the title/time-selection state.
