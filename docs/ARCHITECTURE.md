...
After ranking succeeds, its result keeps Run access in React memory until the user selects `GO TO RUN!`; only then does the browser store the ticket and expiry in `sessionStorage` and navigate to the independently built `/run/` static assets. On non-local hosts, `/run/` starts in `BOOT`, verifies and immediately removes the stored ticket, then shows a horse-free `READY` screen. `START RUN` supplies the user gesture for Web Audio and passes only the verified time to `window.NetSpeedRun.start(runTimeSec)`; audio failure does not block gameplay. Reload, direct re-entry, and BFCache restoration return to `/`, while in-game `RETRY` reuses the verified run time held in memory. Missing, invalid, or expired tickets also return to `/`; service failure remains on an error screen with a manual return link. Raw score and time query parameters are ignored. Localhost keeps the development-only 25/35/45/50-second selector.
...
`public/run/` contains the standalone Net Speed Run v0.1.2 static source and is copied by Vite to `dist/run/` without entering the homepage React bundle. Production proceeds from `BOOT` to `READY`, then starts `INTRO` only after `START RUN`; it never exposes the gameplay test or title screen and ignores raw `time` or `score` query parameters. Web Audio BGM and accepted-jump sound effects are generated locally without external audio assets. The 25/35/45/50-second gameplay test selector remains available only on `localhost`, `127.0.0.1`, and `::1`.
