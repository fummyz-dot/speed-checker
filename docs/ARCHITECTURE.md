...
The public Worker issues a stateless Net Speed Run ticket only after `POST /api/ranking/entries` succeeds. It runtime-validates the private submission response and uses its accepted `entry.scoreTenths` as the score authority without recalculating the score or making another private-service call. The public Worker maps score tenths to Run time tenths using mapping v1: 0 points is 25.0 seconds, 850 points is 50.0 seconds, and higher scores clamp to 50.0 seconds. Standalone `POST /api/run-ticket` issuance is not available. If ticket signing alone fails, ranking remains successful and its response marks Run access unavailable.

Run tickets use HMAC-SHA256 with the dedicated `RUN_TICKET_HMAC_SECRET`, a 30-minute TTL, a cryptographically random 32-byte nonce, and a fixed `net-speed-run` purpose. They contain the mapped Run time but not the raw score, measurement, client IP, connection metadata, or condition label. Verification through `POST /api/run-ticket/verify` is stateless and does not call the ranking service or use persistence.

After ranking succeeds, its result keeps Run access in React memory until the user selects `GO TO RUN!`; only then does the browser store the ticket and expiry plus a separate return context containing only the source measurement ID in `sessionStorage`, then navigate to the independently built `/run/` static assets. On non-local hosts, `/run/` starts in `BOOT`, verifies and immediately removes the stored ticket, then shows the server-verified `runTimeSec` on a horse-free `READY` screen. `START RUN` supplies the user gesture for Web Audio and passes that same verified time to `window.NetSpeedRun.start(runTimeSec)`; audio failure does not block gameplay. Reload, direct re-entry, and BFCache restoration return to `/`, while in-game retry reuses the verified run time held in memory. The production result action returns to `/`, where the one-shot return context restores the matching saved measurement without enabling another ranking submission. Missing, invalid, or expired tickets also return to `/`; service failure remains on an error screen with a manual return link. Raw score and time query parameters are ignored. Localhost keeps the development-only 25/35/45/50-second selector.
...
`public/run/` contains the standalone Net Speed Run v0.1.3 static source and is copied by Vite to `dist/run/` without entering the homepage React bundle. Production proceeds from `BOOT` to `READY`, then starts `INTRO` only after `START RUN`; it never exposes the gameplay test or title screen and ignores raw `time` or `score` query parameters. Web Audio BGM and accepted-jump sound effects are generated locally without external audio assets. The 25/35/45/50-second gameplay test selector remains available only on `localhost`, `127.0.0.1`, and `::1`.
